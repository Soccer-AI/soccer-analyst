...
from detection.utils import bboxes_iou
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                try:
                    ...
                    logger.info(
                        "OOM RuntimeError is raised due to the huge memory cost during label  \
                           assignment. CPU mode is applied in this batch. If you want \
                           to avoid this issue, try to reduce the batch size or image size."
                    )
                    print("OOM RuntimeError is raised due to the huge memory cost during label . \
                           assignment CPU mode is applied in this batch. If you want to \
                           avoid this issue, try to reduce the batch size or image size.")
